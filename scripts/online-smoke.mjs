...
const waitFor = (socket, predicate, timeout = 35000) => new Promise((resolve, reject) => {
...
try {
  ...
  if (questionOne.settings.category !== "football" || questionTwo.settings.category !== "football") throw new Error("Room category is not football");
  if (questionOne.question.category !== "كرة القدم" || questionOne.question.durationMs !== 20_000) throw new Error("Football question or 20-second timer is not synchronized");
