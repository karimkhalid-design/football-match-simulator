...
export default function OnlineQuiz({ onBack }: { onBack: () => void }) {
  ...
  const [screen, setScreen] = useState<"home" | "name" | "create" | "join" | "room" | "leaderboard">("home");
  ...
  useEffect(() => {
    ...
    if (inviteCode) { setCode(inviteCode.toUpperCase()); (window as any).__onlineIntent = "join"; setScreen("name"); }
  ...
  const goName = (next: "create" | "join") => { setError(""); setScreen("name"); setCode(""); setName(""); setCategory("random"); setDifficulty("medium"); (window as any).__onlineIntent = next; };
  ...
  const createRoom = () => emit("create_room", { nickname: name, category, difficulty }, (response) => { if (!response?.ok) return setError("تعذر إنشاء الغرفة"); sessionStorage.setItem("kora-online-token", response.token); setToken(response.token); setState(response.state); setScreen("room"); });
  const joinRoom = () => emit("join_room", { code, nickname: name, token }, (response) => { if (!response?.ok) return setError(response?.error ?? "تعذر دخول الغرفة"); sessionStorage.setItem("kora-online-token", response.token); setToken(response.token); setState(response.state); setScreen("room"); });
  const setReady = () => emit("set_ready", { token });
  const answer = (optionIndex: number) => { if (!state?.ownAnswer && state?.status === "question") emit("answer", { token, optionIndex }); };
  ...
  if (screen === "create") return shell(<section className="online-panel online-create-panel"><img className="online-mini-logo" src={ONLINE_LOGO_URL} alt="" /><p className="online-eyebrow">إعداد الغرفة</p><h1>اختار التحدي</h1><label className="online-label">الفئة<select className="online-select" value={category} onChange={(event) => setCategory(event.target.value as Category)}>{categoryOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><div className="online-difficulty"><span className="online-label">الصعوبة</span>{([ ["easy", "سهل", "🟢"], ["medium", "متوسط", "🟡"], ["hard", "صعب", "🔴"]] as const).map(([value, label, icon]) => <button type="button" key={value} className={difficulty === value ? "active" : ""} onClick={() => setDifficulty(value)}>{icon} {label}</button>)}</div><button type="button" className="online-primary" onClick={createRoom}><Crown /> اعمل الغرفة</button></section>);
