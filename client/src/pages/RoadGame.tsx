...
import { ArrowRight, ChevronLeft, Clock3, Flame, Gauge, Lightbulb, RotateCcw, Share2, Sparkles, Target, Trophy } from "lucide-react";
import ShareResult from "../components/ShareResult";
import { getRoadPlayerForDay, getRoadRandomPlayer, getRoadScore, getRoadVisibleTimeline, isRoadGuessCorrect, ROAD_ROUND_SECONDS, ROAD_SCORE_STEPS, type RoadDifficulty, type RoadPlayer } from "../lib/roadGameData";
...
export default function RoadGame({ onBack }: RoadGameProps) {
  ...
  const visibleTimeline = useMemo(() => player ? getRoadVisibleTimeline(player, hintsUsed) : [], [player, hintsUsed]);
  ...
  const startRound = (selectedMode: Mode = "normal") => {
    const nextPlayer = selectedMode === "daily" ? getRoadPlayerForDay() : getRoadRandomPlayer(player?.id);
  ...
  const revealNext = () => {
    if (hintsUsed >= ROAD_SCORE_STEPS.length) {
      finishGame("revealed");
      return;
    }
  ...
  if (phase === "finished") {
    ...
    return <main className="road-game road-result" dir="rtl">
      ...
      <section className="road-result-shell"><div className={`road-result-badge ${resultReason}`}><Trophy /></div><p className="road-kicker"><Sparkles /> {mode === "daily" ? "PLAYER OF THE DAY" : "سهم اللاعب"}</p><h1>{reasonLabel}</h1><p className="road-result-answer">الإجابة كانت: <strong>{player.arabicName}</strong></p><div className="road-score-panel"><div><small>Score</small><b>{resultScore} / 100</b></div><div><small>Hints Used</small><b>{hintsUsed} / 6</b></div><div><small>Time</small><b>{formatTime(elapsed)}</b></div></div><div className="road-streak-banner"><Flame /> Streak: <b>{streak}</b><span>Best: {bestStreak}</span></div><div className="road-full-timeline"><div className="road-timeline-heading"><span>مسيرة اللاعب</span><small>السهم كامل</small></div><div className="road-timeline road-timeline-full">{player.timeline.map((node) => <div className="road-timeline-node" key={`${node.year}-${node.club}`}><strong>{node.year}</strong><span>{node.club}</span><small>{node.country} · {node.rating}</small></div>)}</div></div><div className="road-result-actions"><button type="button" className="road-primary" onClick={() => startRound(mode)}><RotateCcw /> لاعب جديد</button><ShareResult gameName="الطريق ما يتوهش" eyebrow={mode === "daily" ? "لاعب اليوم · نتيجة سرية" : "الجولة انتهت · نتيجة سرية"} winnerName="أنت" winnerScore={`${resultScore} نقطة`} rows={[{ label: "Score", score: `${resultScore} / 100`, detail: "النتيجة" }, { label: "Hints", score: `${hintsUsed} / 6`, detail: "عدد التلميحات" }, { label: "Time", score: formatTime(elapsed), detail: "الوقت" }, { label: "Streak", score: String(streak), detail: "السلسلة" }]} highlights={["نتيجة سرية", `Hints: ${hintsUsed}`, `Streak: ${streak}`]} accent="#b7ff1a" triggerLabel="مشاركة النتيجة" triggerClassName="road-share-trigger" /></div></section>
    </main>;
  }
  ...
  return <main className="road-game road-play" dir="rtl">
    ...
    <section className="road-play-shell"><div className="road-play-heading"><div><p className="road-kicker"><Gauge /> اقرأ المسيرة واختار</p><h1>مين <em>اللاعب؟</em></h1></div><div className={`road-timer ${seconds <= 15 ? "urgent" : ""}`}><Clock3 /> {formatTime(seconds)}</div></div><div className="road-potential"><span>رصيدك الحالي</span><b>{potentialScore}</b><small>نقطة محتملة</small><div className="road-score-track">{ROAD_SCORE_STEPS.map((score, index) => <i key={score} className={index < hintsUsed ? "active" : ""} />)}</div></div><div className="road-hint-card"><div className="road-hint-label"><Lightbulb /> {player.hints[hintsUsed - 1].label}<span>{hintsUsed} / 6</span></div><p>{player.hints[hintsUsed - 1].text}</p></div><div className="road-timeline-heading"><span>سهم المسيرة</span><small>كل Hint يفتح جزءاً جديداً</small></div><div className="road-timeline">{visibleTimeline.map((node) => <div className={`road-timeline-node ${node.club === "؟" ? "hidden" : "revealed"}`} key={`${node.year}-${node.club}`}><strong>{node.year}</strong><span>{node.club}</span><small>{node.country}{node.rating ? ` · ${node.rating}` : ""}</small></div>)}</div><div className="road-guess-card"><label htmlFor="road-guess">من هو اللاعب؟</label><div className="road-input-row"><input id="road-guess" value={guess} onChange={(event) => setGuess(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") submitGuess(); }} placeholder="اكتب الاسم بالعربي أو الإنجليزي" autoComplete="off" /><button type="button" onClick={submitGuess}>خمن</button></div>{feedback && <p className={feedback.startsWith("صح") ? "correct" : "wrong"}>{feedback}</p>}<button type="button" className="road-reveal" onClick={revealNext}><ChevronLeft /> كشف التالي <span>- {ROAD_SCORE_STEPS[Math.min(hintsUsed, ROAD_SCORE_STEPS.length - 1)] ?? 15} نقطة</span></button></div><div className="road-play-footer"><span><Flame /> {streak} Streak</span><span>{wrongGuesses ? `-${wrongGuesses * 5} خصم` : "كل محاولة محسوبة"}</span></div></section>
  </main>;
}
