...
export default function MenByehbad({ onBackToHub, initialNames }: Props) {
  ...
  const Shell = ({ children }: { children: React.ReactNode }) => <main className="menbyehbad-page" dir="rtl"><div className="menbyehbad-grid" /><div className="menbyehbad-glow" /><header className="menbyehbad-header"><button className="menbyehbad-back" onClick={onBackToHub}><ArrowRight /> كل الألعاب</button><div className="menbyehbad-brand"><span>PARTY FOOTBALL GAME</span><img src={MEN_BYEHBAD_LOGO_URL} alt="شعار مين بيهبد؟" /></div><div className="menbyehbad-round"><Trophy /> {phase === "setup" || phase === "instructions" ? "جاهزين؟" : `${roundIndex + 1} / 10`}</div></header>{children}<footer className="menbyehbad-footer">مين بيهبد؟ · صناعة كريم</footer></main>;

  if (phase === "setup") return <Shell><section className="menbyehbad-setup"><img className="menbyehbad-hero-logo" src={MEN_BYEHBAD_LOGO_URL} alt="مين بيهبد؟" /><p className="menbyehbad-kicker"><Users /> لعبة صحاب · موبايل واحد</p><h1>مين <em>بيهبد؟</em></h1><p className="menbyehbad-copy">متصدقش كل اللي تسمعه… ناقشوا المعلومة وشوفوا مين هيعرف الحقيقة.</p><div className="menbyehbad-panel"><div className="menbyehbad-panel-title"><b>مين هيلعب؟</b><span>{names.length} / 10 لاعبين</span></div><div className="menbyehbad-names">{names.map((name, index) => <label key={index}><span>لاعب {index + 1}</span><div><input value={name} onChange={(event) => updateName(index, event.target.value)} placeholder="اكتب اسم اللاعب" maxLength={18} />{names.length > 3 && <button type="button" aria-label={`حذف اللاعب ${index + 1}`} onClick={() => removePlayer(index)}><X /></button>}</div></label>)}</div><button className="menbyehbad-add" type="button" onClick={addPlayer} disabled={names.length >= 10}><Plus /> إضافة لاعب</button>{setupError && <p className="menbyehbad-error">{setupError}</p>}<button className="menbyehbad-primary" type="button" onClick={startGame}>ابدأ اللعبة <ArrowRight /></button></div></section></Shell>;

  if (phase === "instructions") return <Shell><section className="menbyehbad-card menbyehbad-instructions"><div className="menbyehbad-card-icon"><Eye /></div><p className="menbyehbad-kicker">قواعدها بسيطة</p><h1>ركز كده <em>👀</em></h1><p>هتظهر معلومة عن الكورة. اتناقشوا فيها، وبعدها كل واحد هياخد الموبايل ويختار في السر: حقيقة ولا هبد.</p><div className="menbyehbad-steps"><span><b>01</b> اقرأوا المعلومة</span><span><b>02</b> اتناقشوا 20 ثانية</span><span><b>03</b> كل لاعب يختار في السر</span><span><b>04</b> نكشف الحقيقة ونحسب النقاط</span></div><button className="menbyehbad-primary" onClick={beginRound}>فهمت، ابدأ الجولة <ArrowRight /></button></section></Shell>;

  if (phase === "discussion") return <Shell><section className="menbyehbad-card menbyehbad-statement-card"><div className="menbyehbad-card-top"><span>الجولة {roundIndex + 1} من 10</span><span className="menbyehbad-category">{categoryLabels[round.category]}</span></div><p className="menbyehbad-kicker">ركز كده… متصدقش بسرعة</p><h1>هل المعلومة دي <em>حقيقة؟</em></h1><div className="menbyehbad-statement">{round.statement}</div><div className={`menbyehbad-timer ${seconds <= 5 ? "danger" : ""}`}><Clock3 /><strong>{seconds}</strong><span>ثانية للنقاش</span></div><p className="menbyehbad-talk">اتناقشوا بسرعة! مين شايف إنها هبد؟</p><button className="menbyehbad-secondary" onClick={finishDiscussion}>خلصنا النقاش <ArrowRight /></button></section></Shell>;

  if (phase === "secret") return <Shell><section className="menbyehbad-card menbyehbad-secret"><div className="menbyehbad-lock">🔒</div><p className="menbyehbad-kicker">اختيار سري</p><h1>دور <em>{names[currentPlayer]}</em></h1><p>اختار إجابتك، وبعدها خبّي الشاشة ومرر الموبايل للي بعدك.</p><div className="menbyehbad-secret-buttons"><button className="truth" onClick={() => chooseAnswer(true)}><Check /> حقيقة</button><button className="bluff" onClick={() => chooseAnswer(false)}><X /> هبد</button></div><small>إجابات اللاعبين اللي فاتوا اتقفلت ومش هتظهر.</small></section></Shell>;

  if (phase === "reveal") return <Shell><section className="menbyehbad-card menbyehbad-reveal"><div className="menbyehbad-search">{checking ? "CHECKING…" : "🔍"}</div><p className="menbyehbad-kicker">نكشف الحقيقة؟</p><h1>{checking ? "استنى… بنراجع المعلومة" : "اضغط عشان نعرف"}</h1><p>{checking ? "بنراجع كل تفصيلة كروية…" : "كل الإجابات اتسجلت. مستعدين؟"}</p>{!checking && <button className="menbyehbad-primary" onClick={revealTruth}><Eye /> اكشف الحقيقة</button>}</section></Shell>;

  if (phase === "score") return <Shell><section className="menbyehbad-card menbyehbad-score"><div className={`menbyehbad-verdict ${round.correctAnswer ? "truth" : "bluff"}`}>{round.correctAnswer ? <><Check /> حقيقة</> : <><X /> هبد</>}</div><p className="menbyehbad-kicker">الحقيقة ظهرت</p><h1>{round.correctAnswer ? "طلعت صح يا موسوعة!" : "يا نهار أبيض، دي هبد!"}</h1><div className="menbyehbad-statement small">{round.statement}</div><p className="menbyehbad-explanation">{round.explanation}</p><div className="menbyehbad-round-scores">{names.map((name, index) => <div key={name}><span>{name}</span><b className={roundPoints[index] ? "earned" : ""}>{roundPoints[index] ? "+100" : "+0"}</b><small>{answers[index] === round.correctAnswer ? "إجابة صح" : "إجابة غلط"}</small></div>)}</div><button className="menbyehbad-primary" onClick={nextRound}>{roundIndex >= 9 ? "النتيجة النهائية" : "الجولة الجاية"} <ArrowRight /></button></section></Shell>;

  return <Shell><section className="menbyehbad-card menbyehbad-finished"><div className="menbyehbad-card-icon"><Trophy /></div><p className="menbyehbad-kicker">انتهت اللعبة</p><h1>مين كان <em>موسوعة؟</em></h1><div className="menbyehbad-leaderboard">{leaderboard.map((entry, index) => <div key={entry.name} className={index === 0 ? "winner" : ""}><strong>{index + 1}</strong><span>{entry.name}</span><b>{entry.score}</b></div>)}</div><div className="menbyehbad-titles"><span>🧠 كاشف الهبد: {leaderboard[0]?.name}</span><span>🔥 موسوعة كورة: {leaderboard[0]?.name}</span><span>🤡 بيصدق أي حاجة: {leaderboard[leaderboard.length - 1]?.name}</span></div><button className="menbyehbad-primary" onClick={restart}><RotateCcw /> لعبة جديدة</button></section></Shell>;
}
