...
import { VarDecision, VarRound, VAR_DECISIONS, getNeutralCaseTitle, getNeutralDiscussionText, getRoundTypeLabel, shuffleVarRounds, varRounds } from "../lib/varData";
...
export default function VarGame({ onBackToHub, roundPool }: VarGameProps) {
  ...
  return <main className="var-page" dir="rtl"><div className="var-grid" /><div className={`var-alert var-alert-${phase}`} />
    ...
    <section className="var-round-head"><div><p className="var-kicker"><Flag /> واقعة حقيقية · لقطة جدلية</p><h1>{getNeutralCaseTitle(round)}</h1><p>{round.minute} · {getRoundTypeLabel(round.type)}</p></div><div className="var-judge"><small>الحكم الحالي</small><b>{judge}</b><span>يمسك الموبايل</span></div></section>
    <section className="var-main-card">
      <div className={`var-screen var-screen-${phase}`}><div className="var-screen-top"><span>LIVE REVIEW</span><span><i /> VAR ROOM</span></div>{phase === "watch" && <div className="var-scene var-case-file"><Tv /><span className="var-real-stamp">واقعة حقيقية · بدون فيديو</span><b>{round.match ?? "مباراة موثقة"}</b><strong>{round.teams}</strong><small>{getNeutralDiscussionText(round)}</small><div className="var-player-tags">{round.players?.map((player) => <span key={player}>{player}</span>)}</div><button className="var-watch" onClick={watchClip}><Eye /> اقرأ الحالة وابدأ النقاش</button></div>}{phase === "discussion" && <div className="var-scene"><div className="var-countdown"><Clock3 /><strong>{seconds}</strong><small>ثانية للنقاش</small></div><b>اتناقشوا… إيه قراركم؟</b><small>{getNeutralDiscussionText(round)}</small><span className="var-live-note"><Volume2 /> الحكم فقط يستخدم الموبايل</span></div>}{phase === "var" && <div className="var-checking"><span className="var-replay-lines" /><Tv /><b>VAR CHECKING...</b><small>الـVAR بيراجع اللقطة من أكثر من زاوية قبل كشف القرار.</small><div className="var-progress"><i /></div><strong>{varSeconds}</strong></div>}{(phase === "decision" || phase === "result") && <div className="var-review-result"><span className="var-replay-tag">CASE FILE · VERIFIED REVIEW</span><div className="var-fake-pitch"><span /><i /><b>اقرأ الأدلة</b></div><small>{round.varInfo}</small>{phase === "result" && round.originalDecision && <em>قرار الحكم الأصلي: {round.originalDecision}</em>}</div>}</div>
      <div className="var-decision-panel">{phase === "watch" && <div className="var-panel-message"><ShieldAlert /><b>استنى يا حكم!</b><span>اقرأ ملف الواقعة الأول، وبعدها يبدأ وقت النقاش.</span></div>}{phase === "discussion" && <><div className="var-panel-title"><b>سجّل توقعات الصحاب</b><span>بعد ما تتفقوا، اختار توقع كل لاعب</span></div><PredictionGrid names={names} predictions={predictions} onChange={updatePrediction} options={round.options} /><button className="var-primary" onClick={openVar}><Tv /> افتح الـVAR</button></>}{phase === "var" && <div className="var-panel-message var-panel-checking"><ShieldAlert /><b>استنى يا حكم!</b><span>الإعادة البطيئة شغالة… الـVAR بيفحص كل زاوية.</span><button className="var-primary" onClick={() => setPhase("decision")}>انتقل للقرار</button></div>}{showDecisionControls && <><div className="var-panel-title"><b>{decisionLabel(round)}</b><span>الحكم يختار القرار النهائي</span></div><div className="var-decision-options">{round.options.map((option) => <button key={option} className="var-choice" onClick={() => chooseDecision(option)}><span>{option}</span><ArrowRight /></button>)}</div></>}{phase === "result" && <ResultPanel round={round} selected={selectedDecision} correct={isCorrect} onNext={nextRound} isLast={roundIndex === sessionRounds.length - 1} />}</div>
    </section>
